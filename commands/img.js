...
export default async function img(message, client) {
    ...
    if (args.length === 0) {
        await client.sendMessage(remoteJid, {
            text: '> _No topic was specified. Please follow this example: .img Obito_'
        });
    } else {
        ...
        if (imageUrl.startsWith('http')) {
            ...
            await client.sendMessage(remoteJid, { image: { url: imageUrl }, caption: '> 𝐏𝐨𝐰𝐞𝐫𝐞𝐝 𝐁𝐲🌹 𝐋𝐎𝐑𝐃 𝐎𝐁𝐈𝐓𝐎 𝐓𝐄𝐂𝐇 🌹' });
        } else {
            await client.sendMessage(remoteJid, {
                text: imageUrl
            });
