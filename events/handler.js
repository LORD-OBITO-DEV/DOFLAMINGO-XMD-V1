...
export function messageHandler(bot) {
  ...
  bot.onText(/\/menu/, async (msg) => {
    ...
    if (!isPremium(userId)) {
      ...
      return bot.sendMessage(msg.chat.id, "❌ You're not a premium user. Contact Lord Obito Dev.");
    }
  ...
  bot.onText(/\/disconnect(?: (.+))?/, async (msg, match) => {
    ...
    if (!isPremium(userId)) {
      ...
      return bot.sendMessage(msg.chat.id, "❌ You're not a premium user. Lord Obito Dev.");
    }
